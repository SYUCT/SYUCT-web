...
- **GitHub 统计显示修正。** 首页每次打开都会重新查询 Star / Fork；缓存与备用数据按观测时间比较，旧值不覆盖新值。2026-09-09 静态备用数据为 12 Star、2 Fork，并非实时保证值。
...
- 首页由独立脚本读取 GitHub Star / Fork。每次打开都查询，恢复联网或切回页面时可重试（间隔至少 30 秒、请求超时 8 秒）；失败时保留 HTML、浏览器缓存和 `assets/github-stats.json` 中时间最新的快照。静态备用值仍需维护，不在断网时宣称实时星数。
...
│   ├── github-live-stats.js           # 首页统计刷新与带时间的本地缓存
